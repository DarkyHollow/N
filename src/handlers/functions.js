...
$onlyIf[$voiceId[$authorId]!=;{newEmbed:{description:$nonEscape[$getEmoji[no]]  You need to connect to a voice channel!}{color:Red}}{deleteIn:5s}{ephemeral}]`
    ...
    code: async (d) => {
        ...
        if (content && content?.length) send.content = content.join(';');
        if (d.data.dm === true) {
    ...
    code: async (d) => {
        ...
        if (!eval(d.helpers.checkCondition.solve(d.helpers.mustEscape(condition)))) {
            ...
            if (err?.trim() === '') {
                ...
                if (d.data.interaction && d.data.interaction?.isChatInputCommand() || d.data.interaction?.deferred) {
    ...
    code: async (d) => {
        ...
        try {
            ...
            const interpreted = (await d.interpreter(
                d.client, d.data.interaction, [],
                { code: `$nonEscape[$queue[${(components[2].data.custom_id).split('_')[1]};10;{position}. [{title}]({url}) - {requester.mention}]]`, name: 'NameParser' },
                undefined, true
            ))?.code;
            
            components[1].data.label = `${player?.queue?.length} Songs`;
            components[0].data.disabled = false;
            components[2].data.custom_id = 'queueNext_' + (Number((components[2].data.custom_id).split('_')[1]) + 1) + `_${d.data.interaction?.author?.id}`;
            components[0].data.custom_id = 'queuePrevious_' + (Number((components[0].data.custom_id).split('_')[1]) + 1) + `_${d.data.interaction?.author?.id}`;
            ...
            d.message.edit({
                components: [],
                embeds: [{
                    description: '<:no:1314836229593759806>  An error occurred: ' + err.message,
                    color: 0xFF0000
                }]
            });
        };
    ...
    code: async (d) => {
        ...
        try {
            ...
            const interpreted = (await d.interpreter(
                d.client, d.data.interaction, [],
                { code: `$nonEscape[$queue[${(components[0].data.custom_id).split('_')[1]};10;{position}. [{title}]({url}) - {requester.mention}]]`, name: 'NameParser' },
                undefined, true
            ))?.code;
